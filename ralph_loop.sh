...
# shellcheck disable=SC2034  # Variable is defined for documentation/future use
...
execute_claude_code() {
	...
	if timeout ${timeout_seconds}s $CLAUDE_CODE_CMD <"$PROMPT_FILE" >"$output_file" 2>&1 & then
		local claude_pid=$!
...
main() {
	...
	while true; do
		...
		if [ $exec_result -eq 0 ]; then
			...
			# shellcheck disable=SC2162  # read without -r is intentional for simple input
