...
- **ralph_import.sh** — converts PRD/spec documents to Ralph format; uses `--output-format json` with automatic text fallback for older CLI versions
  ...
  - Metadata filtering + selection (Issue #71): combinable filters `--github-search <query>`, `--github-label <labels>` (comma = AND), `--github-title <pattern>` (`*` wildcard only, rest literal, case-insensitive), `--github-assignee <user|@me|none>`, `--github-milestone <name>`, `--exclude-label <labels>`, `--github-state <open|closed|all>`. One `gh issue list --limit 100` query (server-side where gh supports it; exclude-label and title matching client-side via jq); candidates sorted oldest-first. `--select first|interactive|priority` picks among multiple matches (priority understands bare `P0`–`P9` and `priority: PN` labels, ties → oldest, none → first; interactive falls back to first on non-TTY/EOF). `--dry-run` previews the match table + would-be selection without importing. `--github-issue` is mutually exclusive with all filter/modifier flags; modifiers require ≥1 primary filter. Implemented in `resolve_github_issue_candidates()` / `select_issue_from_candidates()` / `preview_issue_matches()` (replaced `resolve_github_issue_number()`; "first match" is now oldest, not gh's newest-first)
