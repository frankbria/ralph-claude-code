...
class TestRalphStatusBlock:
    def test_defaults(self):
        ...
        assert block.version == 1  # nosec B101  # pytest assertion
        assert block.work_type == WorkType.UNKNOWN  # nosec B101  # pytest assertion
        assert block.exit_signal is False  # nosec B101  # pytest assertion
    ...
    def test_exit_signal_coercion(self):
        ...
        for truthy in ["true", "True", "TRUE", "yes", "1", True, 1]:
            ...
            assert block.exit_signal is True  # nosec B101  # pytest assertion
    ...
    def test_to_ralph_status(self):
        ...
        assert status.completed_task == "Built feature"  # nosec B101  # pytest assertion
        assert status.exit_signal is True  # nosec B101  # pytest assertion
        assert status.status == RalphLoopStatus.COMPLETED  # nosec B101  # pytest assertion

    def test_model_json_schema(self):
        schema = RalphStatusBlock.model_json_schema()
        assert "properties" in schema  # nosec B101  # pytest assertion
        assert "version" in schema["properties"]  # nosec B101  # pytest assertion
...
Some text after'''
        ...
        assert status is not None  # nosec B101  # pytest assertion
        assert status.work_type == WorkType.TESTING  # nosec B101  # pytest assertion
        assert status.completed_task == "Ran tests"  # nosec B101  # pytest assertion
...
```'''
        status = _parse_json_block(text)
        assert status is not None  # nosec B101  # pytest assertion
        assert status.work_type == WorkType.ANALYSIS  # nosec B101  # pytest assertion
        assert status.exit_signal is True  # nosec B101  # pytest assertion

    def test_returns_none_for_no_block(self):
        assert _parse_json_block("no json here") is None  # nosec B101  # pytest assertion

    def test_returns_none_for_invalid_json(self):
        text = '```json\n{invalid json}\n```'
        assert _parse_json_block(text) is None  # nosec B101  # pytest assertion
...
```'''
    ...
    def test_returns_none_for_no_result(self):
        assert _parse_jsonl_result("not jsonl") is None  # nosec B101  # pytest assertion
...
class TestParseTextFallback:
    def test_extracts_fields(self):
        ...
        assert status.work_type == WorkType.TESTING  # nosec B101  # pytest assertion
        assert status.completed_task == "Ran unit tests"  # nosec B101  # pytest assertion
        assert status.exit_signal is True  # nosec B101  # pytest assertion
    ...
    def test_handles_missing_fields(self):
        ...
        assert status.work_type == WorkType.UNKNOWN  # nosec B101  # pytest assertion
        assert status.exit_signal is False  # nosec B101  # pytest assertion

    def test_case_insensitive(self):
        text = "work_type: DEBUGGING\nexit_signal: YES\n"
        status = _parse_text_fallback(text)
        assert status.work_type == WorkType.DEBUGGING  # nosec B101  # pytest assertion
        assert status.exit_signal is True  # nosec B101  # pytest assertion
...
```'''
        ...
        assert status.work_type == WorkType.IMPLEMENTATION  # nosec B101  # pytest assertion
        assert status.completed_task == "JSON wins"  # nosec B101  # pytest assertion
