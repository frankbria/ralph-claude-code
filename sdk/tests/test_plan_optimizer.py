...
class TestTap628Equivalence:
    ...
    def test_permutation_passes(self):
        ...
        assert _validate_equivalence(before, [b, a]) is True  # nosec B101  # pytest assertion

    def test_dropped_task_fails(self):
        a = _task(0, "- [ ] alpha")
        b = _task(1, "- [ ] beta")
        before = frozenset(t.raw_line for t in [a, b])
        assert _validate_equivalence(before, [a]) is False  # nosec B101  # pytest assertion
    ...
    def test_renamed_task_fails(self):
        ...
        assert _validate_equivalence(before, [renamed]) is False  # nosec B101  # pytest assertion

    def test_added_task_fails(self):
        a = _task(0, "- [ ] alpha")
        before = frozenset([a.raw_line])
        b = _task(1, "- [ ] beta")
        assert _validate_equivalence(before, [a, b]) is False  # nosec B101  # pytest assertion
    ...
    def test_same_text_different_raw_line_fails(self):
        ...
        assert _validate_equivalence(before, [same_text_no_meta]) is False  # nosec B101  # pytest assertion
...
class TestParseTasks:
    def test_returns_empty_for_missing_file(self, tmp_path):
        assert parse_tasks(tmp_path / "absent.md") == []  # nosec B101  # pytest assertion

    def test_extracts_checked_state(self, tmp_path):
        plan = tmp_path / "fix_plan.md"
        plan.write_text("## Section\n- [x] done thing\n- [ ] todo thing\n")
        tasks = parse_tasks(plan)
        assert len(tasks) == 2  # nosec B101  # pytest assertion
        assert tasks[0].checked is True  # nosec B101  # pytest assertion
        assert tasks[1].checked is False  # nosec B101  # pytest assertion
        assert tasks[0].section == "## Section"  # nosec B101  # pytest assertion
    ...
    def test_extracts_files_from_backticks_and_paths(self, tmp_path):
        ...
        tasks = parse_tasks(plan)
        assert "src/main.py" in tasks[0].files  # nosec B101  # pytest assertion
        assert "tests/test_main.py" in tasks[0].files  # nosec B101  # pytest assertion
    ...
    def test_extracts_metadata(self, tmp_path):
        ...
        tasks = parse_tasks(plan)
        assert tasks[0].task_id == "t1"  # nosec B101  # pytest assertion
        assert tasks[1].depends == "t1"  # nosec B101  # pytest assertion
    ...
    def test_extracts_resolved_metadata_as_file(self, tmp_path):
        ...
        tasks = parse_tasks(plan)
        assert "src/resolved.py" in tasks[0].files  # nosec B101  # pytest assertion

    def test_strips_metadata_from_clean_text(self, tmp_path):
        plan = tmp_path / "fix_plan.md"
        plan.write_text("## S\n- [ ] do thing <!-- id: x -->\n")
        tasks = parse_tasks(plan)
        assert "<!--" not in tasks[0].text  # nosec B101  # pytest assertion
        assert "do thing" in tasks[0].text  # nosec B101  # pytest assertion
    ...
    def test_ignores_non_task_lines(self, tmp_path):
        ...
        tasks = parse_tasks(plan)
        assert len(tasks) == 1  # nosec B101  # pytest assertion
...
class TestEstimateSize:
    def test_small_for_typo_fix(self):
        assert _estimate_size("fix typo in readme", 1) == 0  # nosec B101  # pytest assertion

    def test_small_for_rename_single_file(self):
        assert _estimate_size("rename helper", 1) == 0  # nosec B101  # pytest assertion

    def test_large_for_three_files(self):
        assert _estimate_size("regular work", 3) == 2  # nosec B101  # pytest assertion

    def test_large_for_keyword(self):
        assert _estimate_size("redesign authentication", 1) == 2  # nosec B101  # pytest assertion

    def test_medium_default(self):
        assert _estimate_size("update logic", 2) == 1  # nosec B101  # pytest assertion


class TestPhaseRank:
    def test_create_phase(self):
        assert phase_rank("create new module") == 0  # nosec B101  # pytest assertion

    def test_implement_phase(self):
        assert phase_rank("implement feature X") == 1  # nosec B101  # pytest assertion

    def test_test_phase(self):
        assert phase_rank("test the new behavior") == 3  # nosec B101  # pytest assertion

    def test_doc_phase(self):
        assert phase_rank("document the API") == 4  # nosec B101  # pytest assertion

    def test_default_modify(self):
        assert phase_rank("change behavior") == 2  # nosec B101  # pytest assertion
...
class TestTopologicalSort:
    def test_empty_edges_returns_identity(self):
        assert _topological_sort(3, []) == [0, 1, 2]  # nosec B101  # pytest assertion

    def test_simple_chain(self):
        # 0 → 1 → 2: order must be 0,1,2
        assert _topological_sort(3, [(0, 1), (1, 2)]) == [0, 1, 2]  # nosec B101  # pytest assertion

    def test_reverse_chain(self):
        # 2 → 1 → 0: order must be 2,1,0
        assert _topological_sort(3, [(2, 1), (1, 0)]) == [2, 1, 0]  # nosec B101  # pytest assertion

    def test_cycle_tolerant(self):
        """Cycle: 0→1→0. All nodes still appear in output."""
        order = _topological_sort(2, [(0, 1), (1, 0)])
        assert sorted(order) == [0, 1]  # nosec B101  # pytest assertion

    def test_partial_cycle(self):
        """Mixed graph: 0→1, plus 2↔3 cycle. 0,1 sort; 2,3 appended."""
        order = _topological_sort(4, [(0, 1), (2, 3), (3, 2)])
        # 0 must precede 1
        assert order.index(0) < order.index(1)  # nosec B101  # pytest assertion
        # All nodes present
        assert sorted(order) == [0, 1, 2, 3]  # nosec B101  # pytest assertion
    ...
    def test_deterministic_when_multiple_roots(self):
        ...
        assert order.index(0) < order.index(2)  # nosec B101  # pytest assertion
        assert order.index(1) < order.index(3)  # nosec B101  # pytest assertion

    def test_ignores_out_of_range_edges(self):
        # Edge (5,6) silently skipped — doesn't crash
        order = _topological_sort(3, [(0, 1), (5, 6)])
        assert sorted(order) == [0, 1, 2]  # nosec B101  # pytest assertion
...
class TestBuildDependencyEdges:
    ...
    def test_metadata_layer_creates_edge(self):
        ...
        assert (0, 1) in edges  # nosec B101  # pytest assertion
    ...
    def test_metadata_takes_precedence_over_import(self):
        ...
        assert (1, 0) not in edges  # nosec B101  # pytest assertion
...
class TestModuleKey:
    def test_unknown_when_no_files(self):
        assert _module_key([]) == "zzz_unknown"  # nosec B101  # pytest assertion

    def test_first_two_components(self):
        assert _module_key(["src/auth/login.py"]) == "src/auth"  # nosec B101  # pytest assertion

    def test_single_component(self):
        assert _module_key(["main.py"]) == "main.py"  # nosec B101  # pytest assertion
...
class TestOptimizePlan:
    def test_missing_file_returns_unchanged(self, tmp_path):
        ...
        assert result.changed is False  # nosec B101  # pytest assertion
        assert "not found" in result.reason  # nosec B101  # pytest assertion
    ...
    def test_zero_unchecked_returns_unchanged(self, tmp_path):
        ...
        assert result.changed is False  # nosec B101  # pytest assertion
        assert "0-1" in result.reason  # nosec B101  # pytest assertion
    ...
    def test_dry_run_does_not_write(self, tmp_path):
        ...
        assert plan.read_text() == original  # untouched  # nosec B101  # pytest assertion
        if result.changed:
            assert "implement" in result.after[0]  # nosec B101  # pytest assertion
            assert "test" in result.after[1]  # nosec B101  # pytest assertion
    ...
    def test_writes_when_order_changes(self, tmp_path):
        ...
        assert result.changed is True  # nosec B101  # pytest assertion
        new = plan.read_text()
        assert new.index("beta task") < new.index("alpha task")  # nosec B101  # pytest assertion
        # Backup created
        assert (tmp_path / "fix_plan.md.pre-optimize.bak").exists()  # nosec B101  # pytest assertion
        assert result.dependency_count >= 1  # nosec B101  # pytest assertion
    ...
    def test_no_change_returns_unchanged(self, tmp_path):
        ...
        result = optimize_plan(plan, project_root=tmp_path)
        assert result.changed is False  # nosec B101  # pytest assertion
        assert "unchanged" in result.reason  # nosec B101  # pytest assertion
    ...
    def test_preserves_checked_tasks_position(self, tmp_path):
        ...
        assert "- [x] already done" in new  # nosec B101  # pytest assertion
