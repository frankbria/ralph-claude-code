...
class TestTaskInput:
    def test_from_ralph_dir(self, project_dir):
        ...
        assert "login form" in task.prompt  # nosec B101  # pytest assertion
        assert "Add login form" in task.fix_plan  # nosec B101  # pytest assertion
        assert "npm test" in task.agent_instructions  # nosec B101  # pytest assertion

    def test_from_ralph_dir_missing(self, tmp_path):
        ralph_dir = tmp_path / ".ralph"
        ralph_dir.mkdir()
        task = TaskInput.from_ralph_dir(str(ralph_dir))
        assert task.prompt == ""  # nosec B101  # pytest assertion
        assert task.fix_plan == ""  # nosec B101  # pytest assertion
    ...
    def test_from_task_packet(self):
        ...
        assert task.task_packet_id == "task-123"  # nosec B101  # pytest assertion
        assert task.prompt == "Build feature X"  # nosec B101  # pytest assertion
    ...
    def test_model_json_schema(self):
        ...
        assert "prompt" in schema["properties"]  # nosec B101  # pytest assertion
...
class TestTaskResult:
    def test_to_signal(self):
        ...
        assert signal["type"] == "ralph_result"  # nosec B101  # pytest assertion
        assert signal["loop_count"] == 5  # nosec B101  # pytest assertion
        assert signal["task_result"]["EXIT_SIGNAL"] is True  # nosec B101  # pytest assertion
...
class TestRalphAgent:
    def test_init(self, project_dir, config):
        ...
        assert agent.loop_count == 0  # nosec B101  # pytest assertion
        assert agent.ralph_dir == project_dir / ".ralph"  # nosec B101  # pytest assertion
    ...
    async def test_should_exit_requires_dual_condition(self, project_dir, config):
        ...
        assert await agent.should_exit(status, 1) is False  # nosec B101  # pytest assertion

        # EXIT_SIGNAL + completion phrase = exit
        status2 = RalphStatus(exit_signal=True, progress_summary="All tasks complete")
        assert await agent.should_exit(status2, 2) is True  # nosec B101  # pytest assertion

    @pytest.mark.asyncio
    async def test_check_rate_limit_ok(self, project_dir, config):
        agent = RalphAgent(config=config, project_dir=project_dir)
        assert await agent.check_rate_limit() is True  # nosec B101  # pytest assertion

    @pytest.mark.asyncio
    async def test_check_circuit_breaker_ok(self, project_dir, config):
        agent = RalphAgent(config=config, project_dir=project_dir)
        assert await agent.check_circuit_breaker() is True  # nosec B101  # pytest assertion
    ...
    async def test_check_circuit_breaker_open(self, project_dir, config):
        ...
        assert await agent.check_circuit_breaker() is False  # nosec B101  # pytest assertion
    ...
    def test_parse_jsonl_response(self, project_dir, config):
        ...
        assert status.work_type == WorkType.IMPLEMENTATION  # nosec B101  # pytest assertion
        assert status.completed_task == "Added form"  # nosec B101  # pytest assertion
        assert status.exit_signal is False  # nosec B101  # pytest assertion
        assert agent.session_id == "sess-123"  # nosec B101  # pytest assertion

    def test_parse_text_fallback(self, project_dir, config):
        agent = RalphAgent(config=config, project_dir=project_dir)
        text = "Some output...\nWORK_TYPE: TESTING\nEXIT_SIGNAL: true\n"
        status = agent._parse_response(text, 0)
        assert status.work_type == WorkType.TESTING  # nosec B101  # pytest assertion
        assert status.exit_signal is True  # nosec B101  # pytest assertion

    def test_parse_error_return_code(self, project_dir, config):
        agent = RalphAgent(config=config, project_dir=project_dir)
        status = agent._parse_response("", 1)
        assert status.status == RalphLoopStatus.ERROR  # nosec B101  # pytest assertion
    ...
    def test_get_tool_definitions(self, project_dir, config):
        ...
        assert len(tools) == 4  # nosec B101  # pytest assertion
        # No handler in output (SDK registration doesn't need it)
        for tool in tools:
            assert "handler" not in tool  # nosec B101  # pytest assertion
    ...
    def test_build_claude_command(self, project_dir, config):
        ...
        assert cmd[0] == "claude"  # nosec B101  # pytest assertion
        assert "--agent" in cmd  # nosec B101  # pytest assertion
        assert "ralph" in cmd  # nosec B101  # pytest assertion
        assert "-p" in cmd  # nosec B101  # pytest assertion
        assert "--output-format" in cmd  # nosec B101  # pytest assertion
        assert "--max-turns" in cmd  # nosec B101  # pytest assertion
...
class TestExtractSessionId:
    ...
    def test_tokens_extracted_from_usage_nested(self, project_dir, config):
        ...
        assert agent.session_id == "abc-123"  # nosec B101  # pytest assertion
        assert agent._last_tokens_in == 1234  # nosec B101  # pytest assertion
        assert agent._last_tokens_out == 567  # nosec B101  # pytest assertion

    def test_missing_usage_does_not_raise(self, project_dir, config):
        agent = self._agent(project_dir, config)
        stdout = json.dumps({"type": "result", "session_id": "abc-456"})
        agent._extract_session_id(stdout)
        assert agent.session_id == "abc-456"  # nosec B101  # pytest assertion
        assert agent._last_tokens_in == 0  # nosec B101  # pytest assertion
        assert agent._last_tokens_out == 0  # nosec B101  # pytest assertion
    ...
    def test_top_level_input_tokens_ignored(self, project_dir, config):
        ...
        agent._extract_session_id(stdout)
        assert agent._last_tokens_in == 1  # nosec B101  # pytest assertion
        assert agent._last_tokens_out == 2  # nosec B101  # pytest assertion
...
class TestTAP1104AgentMode:
    ...
    def test_use_agent_field_removed(self):
        ...
        cfg = RalphConfig()
        assert not hasattr(cfg, "use_agent")  # nosec B101  # pytest assertion
        # And the export round-trip must not include the legacy key.
        assert "useAgent" not in cfg.to_dict()  # nosec B101  # pytest assertion

    def test_command_always_emits_agent(self, project_dir, config):
        agent = self._agent(project_dir, config)
        cmd = agent._build_claude_command("hello")
        assert "--agent" in cmd  # nosec B101  # pytest assertion
        assert cmd[cmd.index("--agent") + 1] == config.agent_name  # nosec B101  # pytest assertion

    def test_command_never_emits_allowedtools(self, project_dir, config):
        config.allowed_tools = ["Read", "Write", "Bash(git *)"]
        agent = self._agent(project_dir, config)
        cmd = agent._build_claude_command("hello")
        assert "--allowedTools" not in cmd  # nosec B101  # pytest assertion

    def test_min_version_is_2_1_0(self):
        assert RalphConfig().claude_min_version == "2.1.0"  # nosec B101  # pytest assertion
...
class TestModelRouting:
    ...
    def test_routing_disabled_uses_config_model(self, project_dir, config):
        ...
        assert "--model" in cmd  # nosec B101  # pytest assertion
        assert cmd[cmd.index("--model") + 1] == "claude-sonnet-4-6"  # nosec B101  # pytest assertion
    ...
    def test_routing_enabled_trivial_to_haiku(self, project_dir, config):
        ...
        assert cmd[cmd.index("--model") + 1] == config.model_map_trivial  # nosec B101  # pytest assertion
    ...
    def test_routing_enabled_architectural_to_opus(self, project_dir, config):
        ...
        assert cmd[cmd.index("--model") + 1] == config.model_map_architectural  # nosec B101  # pytest assertion
    ...
    def test_extract_next_task_skips_checked(self, project_dir, config):
        ...
        assert agent._extract_next_task_text(task) == "Real next task"  # nosec B101  # pytest assertion

    def test_extract_next_task_empty_when_no_unchecked(self, project_dir, config):
        agent = self._agent(project_dir, config)
        task = TaskInput(fix_plan="- [x] Done\n- [x] Also done\n")
        assert agent._extract_next_task_text(task) == ""  # nosec B101  # pytest assertion
