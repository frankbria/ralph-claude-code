...
# Ralph Enable CI for Codex CLI - Non-Interactive Version for Automation
# Adds Ralph+Codex configuration with sensible defaults, no prompts.
# Parallel to ralph_enable_ci.sh (Claude Code)
#
# Usage:
#   ralph-codex-enable-ci                           # Sensible defaults
#   ralph-codex-enable-ci --from github             # Import from GitHub Issues
#   ralph-codex-enable-ci --project-type typescript  # Override detection
#   ralph-codex-enable-ci --json                     # Machine-readable output
#
# Version: 0.1.0
...
show_help() {
    cat << EOF
Ralph Enable CI for Codex CLI - Non-Interactive Setup

Usage: ralph-codex-enable-ci [OPTIONS]
...
Examples:
    ralph-codex-enable-ci
    ralph-codex-enable-ci --from github
    ralph-codex-enable-ci --project-type typescript --json
...
# Generated by: ralph-codex-enable-ci
...
RALPHRCEOF
    ...
    if [[ "$JSON_OUTPUT" == "true" ]]; then
        ...
        for f in .ralph/PROMPT.md .ralph/fix_plan.md .ralph/AGENT.md .ralphrc.codex; do
        ...
        echo "✅ Ralph+Codex enabled for '$project_name'"
        echo "   Type: $project_type | Engine: codex | Tasks: $TASK_SOURCE"
        echo ""
        echo "Run: ralph-codex --monitor"
    fi
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -v|--version) echo "ralph-codex-enable-ci version $VERSION"; exit 0 ;;
